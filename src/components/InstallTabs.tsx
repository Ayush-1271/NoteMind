...
type Tab = "curl" | "npm" | "bun" | "brew" | "paru";
...
export function InstallTabs() {
  ...
  const tabs: Tab[] = ["curl", "npm", "bun", "brew", "paru"];

  return (
    <div className="w-full max-w-4xl border border-white/10 rounded-lg bg-[#0E0E0E] mt-8 mb-16 overflow-hidden">
      <div className="flex items-center border-b border-white/5 px-2 bg-[#121212]">
        {tabs.map((tab) => (
          <button
            key={tab}
      ...
      <div className="p-5 flex items-center justify-between">
        <code className="text-gray-400 font-mono text-sm group">
          {commands[activeTab]} <span className="text-white/20 select-none opacity-0 group-hover:opacity-100 transition-opacity">|</span>
        </code>
        <button
          onClick={handleCopy}
